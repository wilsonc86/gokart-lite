...
echo "Begin to build ssslite with tag '${tag}' for '$env' environment"
...
if [[ "$action" = "all" ]] || [[ "$action" = "build"  ]]; then
    if [[ "$env" != "prod" ]]; then
    	npm install
        if [[ $? -ne 0 ]]; then
    	    echo "Install third parth javascript modules failed"
        	exit 1
        fi
    	source venv/bin/activate && npm run vendor_dev && npm run app_dev
        if [[ $? -ne 0 ]]; then
    	    echo "Compile sss failed"
        	exit 1
        fi
    fi

    if [[ "$env" = "prod" ]]; then
    	docker image build -t dbcawa/ssslite:${tag} -f Dockerfile .
    else
    	docker image build -t dbcawa/ssslite:${tag} -f Dockerfile_dev .
    fi
    if [[ $? -ne 0 ]]; then
...
if [[ "$action" = "all" ]] || [[ "$action" = "test"  ]]; then
    docker container run --publish 30019:8080 --env DEBUG=debug --env ENV_TYPE=${env} --env DIST_TYPE=${disttype} dbcawa/ssslite:${tag}
    if [[ $? -ne 0 ]]; then
...
if [[ "$action" = "all" ]] || [[ "$action" = "push"  ]]; then
    ...
    docker image push dbcawa/ssslite:${tag}
    if [[ $? -ne 0 ]]; then
